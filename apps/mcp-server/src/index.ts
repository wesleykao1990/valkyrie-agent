...
const tools = [
  ...
  tool("memory_search", "Search accepted project knowledge in read-only mode.", { projectId: stringProp("Project ID"), query: stringProp("Search query") }, ["projectId", "query"]),
  ...
  tool("memory_promote", "Promote a reviewed proposal into accepted Markdown.", { proposalId: stringProp("Proposal ID") }, ["proposalId"]),
...
async function callTool(name: string, args: any): Promise<unknown> {
  switch (name) {
    ...
    case "memory_promote": return api(`/api/memory/proposals/${encodeURIComponent(args.proposalId)}/resolve`, { method: "POST", body: { decision: "promote" } });
