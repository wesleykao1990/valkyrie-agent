...
export interface RouteDecision {
  ...
  executionShape: EngineeringExecutionShape | null;
  supported: boolean;
  reason: string;
}
...
export type WorkClass = "research" | "engineering" | "meeting" | "knowledge" | "operations";
...
export function routeTask(input: StartRunInput): RouteDecision {
  ...
  const engineering = /\b(?:implement|fix|debug|refactor|code|test|parser|api|migration|schema|repository|pull request|review)\b/u.test(text);
  const research = !engineering && /\b(?:research|benchmark|investigate|experiment|compare models)\b/u.test(text);
  const monitoring = !engineering && !research && /\b(?:remind|monitor|weekly|daily brief|watch)\b/u.test(text);
  const workClass: WorkClass = research ? "research" : monitoring ? "operations" : "engineering";
  const role: AgentRole = research ? "research-lead" : monitoring ? "valkyrie" : "engineering-lead";
  ...
  if (input.runtime) {
    ...
    return {
      ...
      executionShape: null,
      supported: true,
      reason: "Explicit runtime selection retained for fixed and test paths",
    };
  }
  ...
  if (research) {
    return {
      ...
      executionShape: null,
      supported: false,
      reason: "Non-engineering research requires the future Research Lead selector; no keyword selects Prime",
    };
  }
  if (monitoring) {
    return {
      ...
      executionShape: null,
      supported: true,
      reason: "Existing monitoring requests remain with the Hermes gateway path",
    };
  }
  const assessment = assessEngineeringRequest({
  ...
  return {
    workClass,
    role,
    runtime: assessment.decision.shape === "direct" ? "codex" : "atomic",
    executionShape: assessment.decision.shape,
    supported: true,
    reason: `Legacy fixed/demo launch mapped from risk-based engineering assessment ${assessment.decision.shape}; future general launch must consume the durable assessment`,
  };
}
